...
function createFloor() {
  floor = Bodies.rectangle(CONFIG.worldWidth / 2, innerHeight - CONFIG.floorHeight / 2, CONFIG.worldWidth, CONFIG.floorHeight, {
    isStatic: true,
    friction: 1,
    restitution: 0,
...
function buildTower() {
  ...
  bodies.push(Bodies.circle(x, baseY - 420, 28, {
    density: 0.0015,
    friction: 0.35,
    restitution: 0.08,
...
function startDrag(event) {
  ...
  if (Vector.magnitude(Vector.sub(point, hero.position)) > CONFIG.heroRadius * 1.7) return;
...
function finishDrag(event) {
  if (!state.dragging || !hero) return;
  if (event?.pointerId !== undefined && state.pointerId !== null && event.pointerId !== state.pointerId) return;
  ...
  const velocity = Vector.mult(direction, launchDistance * CONFIG.launchPower);
...
function updateCamera() {
  ...
  if (hero && state.launched && hero.position.x > 580) {
...
window.addEventListener("blur", () => {
  if (state.dragging) finishDrag({ pointerId: state.pointerId });
});
...
window.addEventListener("resize", () => resetWorld(false));
