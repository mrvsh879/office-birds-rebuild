...
function renderShotRack() {
  shotRack.innerHTML = '';
  for (let i = 0; i < LEVELS[levelIndex].shots; i++) {
    const dot = document.createElement('span');
    dot.className = `shot-dot${i < shotsUsed ? ' used' : ''}`;
    shotRack.appendChild(dot);
  }
}
function updateHud() {
...
function createProjectile() {
  ...
  released=false; detached=false; respawnQueued=false; lowSpeedFrames=0; shotStartedAt=0;
...
function setupMouse() {
  ...
  Events.on(mouseConstraint,'startdrag',e=>{ if(paused||levelFinished||e.body!==ball||detached)return; released=false; setStatus('Натяжение'); });
  Events.on(mouseConstraint,'enddrag',e=>{ if(paused||levelFinished||e.body!==ball||detached)return; const stretch=Vector.magnitude(Vector.sub(ball.position,anchor)); if(stretch<45){setStatus('Потяни сильнее');return;} released=true; shotsUsed++; shotStartedAt=performance.now(); setStatus('Резинка отпущена'); updateHud(); });
  World.add(engine.world,mouseConstraint); render.mouse=mouse;
}

function loadLevel(index) {
  paused=false; engine.timing.timeScale=1; pausePanel.classList.remove('show'); pauseBtn.textContent='Пауза';
...
function queueProjectile() {
  ...
  setTimeout(()=>{ if(levelFinished)return; if(ball)World.remove(engine.world,ball); if(sling)World.remove(engine.world,sling); remainingShots()>0?createProjectile():finishLevel(false); },350);
}
function defeatTarget(t) { if(!t||t.defeated)return; t.defeated=true; t.render.fillStyle='#7f8c8d'; t.render.strokeStyle='#34495e'; score+=100; updateHud(); if(aliveTargets()===0)finishLevel(true); }
...
function finishLevel(won) {
  ...
  if(won) {
    ...
    resultTitle.textContent='Уровень пройден!'; resultStars.textContent='★'.repeat(stars)+'☆'.repeat(3-stars); resultText.textContent=`Счёт: ${finalScore}. Осталось выстрелов: ${remainingShots()}.`; nextBtn.hidden=false; nextBtn.textContent=levelIndex===LEVELS.length-1?'К уровням':'Следующий уровень'; setStatus('Уровень пройден');
  } else {
...
function renderLevelMenu() {
  const total=progress.levels.reduce((s,l)=>s+l.stars,0); totalStarsEl.textContent=`${total}/${LEVELS.length*3}`; levelGrid.innerHTML='';
  LEVELS.forEach((l,i)=>{ const unlocked=i<progress.unlocked; const rec=progress.levels[i]; const btn=document.createElement('button'); btn.className='level-tile'; btn.disabled=!unlocked; btn.innerHTML=`<span class="level-number">${i+1}</span><strong>${l.name}</strong><span class="level-stars">${unlocked?'★'.repeat(rec.stars)+'☆'.repeat(3-rec.stars):'🔒'}</span><small>${rec.bestScore?`Рекорд: ${rec.bestScore}`:'Не пройден'}</small>`; btn.onclick=()=>loadLevel(i); levelGrid.appendChild(btn); });
}
...
function togglePause(force) {
  if(levelFinished||levelMenu.classList.contains('show'))return;
  paused=typeof force==='boolean'?force:!paused; engine.timing.timeScale=paused?0:1; pausePanel.classList.toggle('show',paused); pauseBtn.textContent=paused?'Продолжить':'Пауза';
}

function drawTrajectory(ctx) {
  if(!mouseConstraint?.body || mouseConstraint.body!==ball || detached || paused)return;
  const pull=Vector.sub(anchor,ball.position); const distance=Vector.magnitude(pull); if(distance<25)return;
  let p={...ball.position}; let v=Vector.mult(Vector.normalise(pull),Math.min(distance,320)*.125); const g=.278;
  ctx.save();
  for(let i=0;i<34;i++){ p.x+=v.x; p.y+=v.y; v.y+=g; if(i%2)continue; ctx.beginPath(); ctx.fillStyle=`rgba(255,221,112,${.9-i/45})`; ctx.arc(p.x,p.y,4,0,Math.PI*2); ctx.fill(); }
  ctx.restore();
}
...
Events.on(engine,'beforeUpdate',()=>{
  ...
  if(detached&&!respawnQueued){ const speed=Vector.magnitude(ball.velocity); lowSpeedFrames=speed<.75?lowSpeedFrames+1:0; const outside=ball.position.x>innerWidth+220||ball.position.x<-220||ball.position.y>innerHeight+260; const timeout=shotStartedAt&&performance.now()-shotStartedAt>6500; if(outside||lowSpeedFrames>24||timeout)queueProjectile(); }
  updateHud();
});

Events.on(render,'afterRender',()=>{ if(!anchor)return; const ctx=render.context; drawTrajectory(ctx); ctx.save(); ctx.lineCap='round';ctx.lineJoin='round';ctx.strokeStyle='#75421f';ctx.lineWidth=22;ctx.beginPath();ctx.moveTo(anchor.x,floorY-5);ctx.lineTo(anchor.x,anchor.y+10);ctx.moveTo(anchor.x,anchor.y+20);ctx.lineTo(anchor.x-45,anchor.y-45);ctx.moveTo(anchor.x,anchor.y+20);ctx.lineTo(anchor.x+45,anchor.y-45);ctx.stroke(); if(!detached&&ball){ctx.strokeStyle='#3b2417';ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(anchor.x-45,anchor.y-45);ctx.lineTo(ball.position.x,ball.position.y);ctx.lineTo(anchor.x+45,anchor.y-45);ctx.stroke();} ctx.restore(); });
...
window.addEventListener('keydown',e=>{ if(e.key.toLowerCase()==='r')loadLevel(levelIndex); if(e.key==='Escape')togglePause(); if(e.code==='Space'){e.preventDefault();togglePause();} });
