...
function createScene() {
  ...
  floor = Bodies.rectangle(innerWidth / 2, floorY + 45, innerWidth + 400, 90, {
    isStatic: true,
    friction: 1,
  ...
  World.add(engine.world, [floor, ball, sling, mouseConstraint]);
...
Events.on(engine, "beforeUpdate", () => {
  ...
  if (released && !detached && sling) {
    ...
    // Резинка разгоняет шар до точки рогатки, после чего отпускает его.
  ...
  if (detached && (ball.position.x > innerWidth + 250 || ball.position.y > innerHeight + 300)) {
    window.setTimeout(resetBallOnly, 250);
    detached = false;
  }
...
Events.on(render, "afterRender", () => {
  ...
  // Две резинки идут к шару, пока он прикреплён.
  if (!detached && ball) {
...
resetBtn.addEventListener("click", createScene);
